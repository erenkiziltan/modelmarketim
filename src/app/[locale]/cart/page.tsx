  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center">
        <ShoppingBag className="h-14 w-14 text-zinc-300 mx-auto mb-4" />
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 flex flex-col gap-4">
          {items.map(item => {
            ...
            return (
              <div key={`${item.product.id}-${JSON.stringify(item.selectedVariants)}`} className="flex gap-4 bg-white rounded-xl border p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <button
                      ...
                      className="w-7 h-7 rounded border text-sm font-medium text-zinc-700 hover:bg-zinc-100"
                    >−</button>
                ...
                <div className="flex flex-col items-end justify-between">
                  ...
                  <p className="font-bold text-zinc-900 text-sm">{formatPrice(item.product.price * item.quantity)}</p>
                </div>
              </div>
            )
          })}
        ...
        <div className="lg:w-72">
          <div className="bg-white rounded-xl border p-6 sticky top-24">
            <h2 className="font-bold text-zinc-900 mb-4">Sipariş Özeti</h2>
            ...
            <div className="flex justify-between font-bold text-zinc-900 text-lg border-t pt-4 mb-6">
              <span>{t('total')}</span>
              <span>{formatPrice(total)}</span>
            </div>
