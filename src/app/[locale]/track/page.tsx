...
export default function TrackPage() {
  ...
  const statusSteps: { key: OrderStatus; label: string; icon: React.ElementType }[] = [
    ...
    { key: 'delivered', label: t('status_delivered'), icon: Package },
  ]
      ...
      {order && (
        <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-500 mb-0.5">{t('label_order_number')}</p>
              <p className="font-mono font-bold text-slate-900">{order.order_number}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-slate-500 mb-0.5">{t('label_date')}</p>
          ...
          <div className="px-6 py-8">
            {isCancelled ? (
              <div className="text-center py-4">
                ...
                <p className="font-semibold text-red-600">{t('status_cancelled')}</p>
              </div>
            ) : (
          ...
          <div className="px-6 pb-6 grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
            <div>
              <p className="text-xs text-slate-400 mb-0.5">{t('label_customer')}</p>
